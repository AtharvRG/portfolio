...
function Digit({ place, value, height, digitStyle }: DigitProps) {
    const isDot = place === '.';
    const numericPlace = isDot ? 1 : (place as number);
    ...
    if (isDot) {
        return (
            <span
                className="relative inline-flex items-center justify-center pt-2"
                style={{ height, width: 'fit-content', paddingBottom: '0.1em', ...digitStyle }}
            >
                .
            </span>
        );
    ...
    const defaultStyle: React.CSSProperties = {
        ...
        paddingTop: '0.05em', // Nudge numbers down slightly so they don't clip the top edge
        marginTop: '-0.05em' // Offset the visual shift
    };
...
interface CounterProps {
    ...
    className?: string; // Appended property for custom tw classes
}
