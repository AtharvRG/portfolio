...
const createAnimatedNodes = (args: ReducedValue, word: string, index: number): ReducedValue => {
  ...
  const className = cn(
    // Start invisible; use animation-fill-mode:forwards so the final keyframe persists
    "inline-block opacity-0 transition-all ease-in-out [animation-fill-mode:forwards]",
    {
...
export default function WaveReveal({
  ...
  return (
    <div
      className={cn(
        "relative flex font-wistle italic bold flex-wrap justify-center whitespace-pre px-2 text-4xl font-black md:px-6 md:text-7xl",
        className,
      )}
      style={{
        color: '#f8e7ca'
